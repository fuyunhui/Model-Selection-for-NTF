...
if ~isfield(ops, 'maxtime'),    ops.maxtime = 1800;              end
if ~isfield(ops, 'tol'),        ops.tol = 1e-6;                 end
GG = cell( dim, 1 );
for d = 1:dim
    GG{d} = H{d}'*H{d};
end
        ...
        for itr = 1:ops.maxitr
           ...
           his.err(itr) = Loss( Y, H, ops, normY, dim );
                ...
                if toc > ops.maxtime || ( itr>1 && ...
                        abs( sqrt(his.err(itr))/normY - sqrt(his.err(itr-1))/normY ) < ops.tol )
                    his.err(itr+1:end) = [];
                    his.time(itr+1:end) = [];
                    his.admm(:,itr+1:end) = [];
                    break; 
...
%         ops.mu = 1e-7 + .1*his.err(itr);
        ...
        Yt = Ym; V = cell( size(Ym) ); %ops.mu = 1e-7;
        for d = 1:dim, Yt{d}( isnan(Ym{d}) ) = 0;   end
        ...
        for itr = 1:2*ops.maxitr
            ...
            his.time(itr) = toc;
            his.err(itr) = Loss( Y, H, ops, normY, dim );
            %ops.mu = 1e-7 + .1*(sqrt(his.err(itr))/normY);
